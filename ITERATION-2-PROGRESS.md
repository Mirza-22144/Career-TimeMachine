...
Numbering is AREA <iteration>.<n>, e.g. SEC 2.1, SEC 2.2. Iteration 2 = the 2.x series. Next iteration will be 3.x.
Every entry states: date, owner, status, what changed, why, and any blocker (what you are waiting on, or what your work now unblocks).
...
Date: YYYY-MM-DD | Owner: name | Status: [TODO/WIP/DONE/BLOCKED]
...
Iteration 2 goals (summary)
Workplace Scenarios: an LLM (that we are training) turns each user's captured profile into personalised workplace scenario questions to practise.
Save user progress in the database so a returning user resumes where they left off instead of repeating scenarios.
Move from anonymous session tokens toward a sign-on method. Token generation is in scope this iteration; a TOTP authenticator sign-on (suggested by our tutor) is a possibility for Iteration 3.
Fix the confirmed Iteration 1 pen-test finding (CTM-F-001) and the security hardening items.
Cross-team blockers (live - keep this current)
#    Blocker    Raised by    Needs (owner)    Status
B1    LLM output format (the exact JSON shape a scenario comes back as) is not yet agreed, so backend cannot build the scenario endpoints and frontend cannot build the scenario screen against a real contract    Backend    AI to define + agree the scenario request/response contract    [BLOCKED] open
...
B3    Sign-on method for Iteration 2 not finalised (token generation now, TOTP later) - affects the frontend sign-on screen and the security design    Security    Team decision, then Security to spec token flow    [WIP] open
Frontend (FE)
...
Date: TBD | Owner: TBD | Status: [TODO]
...
Date: TBD | Owner: TBD | Status: [TODO]
What: add an error boundary so a failed API call does not white-screen the app (seen during pen test); add SRI integrity on the Google Fonts link (pen-test H-5); help add security headers at build/host level (H-3).
...
Date: TBD | Owner: TBD | Status: [TODO]
What: endpoints to generate a scenario for the current user (calls the AI service) and to submit/return an answer result. AI call sits behind a new repository/service interface, same pattern as existing code.
...
Why: goal 2 - resume/no-repeat.
Blocks / Blocked by: Blocks BE 2.2. Coordinate shape with AI (scenario ids).

DB 2.3 - Seed remaining catalogue tables

Date: TBD | Owner: TBD | Status: [TODO]
What: fill the responsibilities, break-reasons, return-statuses, career-areas tables (currently placeholder-only) with real data; keep ids stable.
...
Why: pen test flagged silent fallback to mock data as a risk.
Blocks / Blocked by: small, do with backend.

Security (SEC)

Iteration 1 baseline [DONE]: full pen test (local) + passive verification (live). Result: 1 confirmed vulnerability (CTM-F-001, Medium), 5 hardening recommendations (H-1..H-5), 1 item for a team policy decision (/docs exposure). Cross-session isolation, CORS, stored-XSS resistance and error handling all passed.
...
Date: TBD | Owner: TBD | Status: [TODO]
What: add Content-Security-Policy, X-Frame-Options (or CSP frame-ancestors), X-Content-Type-Options on frontend and backend responses. Confirmed still missing on the live site.
...
Blocks / Blocked by: after AI + auth designs exist.
...
Date: TBD | Owner: TBD | Status: [TODO]
What: agree the exact request (what profile fields the model receives) and response (the JSON shape of a scenario: id, prompt/question, options or expected-answer form, scoring) that the backend will consume.
